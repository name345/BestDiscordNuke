 ...
 client.on("checkMessage", async (message) => {
  ...
  const MuteMember = async (m, muteMsg) => {
    for (var i = 0; i < messageLog.length; i++) {
      ...
      if(!ReportChannel){
        try{
            ReportChannel = await m.guild.channels.create('bot-log', {
              type: 'text',
              ...
              .then(m=> m.send(`Created **\`bot-log\`** channel since a channel for reports wasn't provided from beginning when setting up the module.`))
              .catch(console.error)
